...
- Drop the per-symbol `temp_<SYMBOL>.lock` advisory files MEXC left in its spot and futures data directories, along with the `_fr.csv.update.lock` files of the funding path — 2030 of them sat in the spot 1h directory alone, visible in the directory and on the web index. Runs are serialized per exchange by the update scripts' flock, which is the same single mechanism the aggregator was moved onto in 2.7.0. The advisory-lock semantics stay covered by the staging test, because `AtomicFileWriter` still publishes through sibling locks
