...
## [2.7.7](https://github.com/vitakot/crypto_data_downloader/releases/tag/v2.7.7) (2026-09-06)
- Retry transient network faults in the Bybit, Lighter and Hyperliquid per-symbol loops. They retried only rate-limit rejections, so a reset connection, a lost DNS answer or a timed-out socket ended the symbol on its first attempt (the log even said "attempt 1/5") and left its CSV cut at that point: Bybit reset six symbols mid-1m-history, one Hyperliquid run lost 73 symbols to `Host not found`, Lighter three. Every loop re-reads the CSV tail before each attempt, so a retry resumes exactly where the fault cut in. A venue's own rejection is still not retried
- Cross a relisting gap in Bybit spot history instead of stopping at it. When no candle exists at or after `start`, the venue answers with its window of the newest candles *before* `start`, and the client read a batch that wholly predates `from` as the end of the history. For a symbol delisted and later relisted that is exactly what `from` inside the gap draws, so the 1m history of SHIBUSDT froze at 2021-11-29, LUNC/USTC at 2022-05-12, ZKJ at 2024-06-10 and GRAM at 2026-06-16 — and the backup, written by the same code, was frozen the same way. `end` anchors the venue's window, so the client now probes forward in windows of exactly `limit` slots, skips a proven-empty window whole and resumes at the first window holding data, which is complete by construction

## [2.7.8](https://github.com/vitakot/crypto_data_downloader/releases/tag/v2.7.8) (2026-09-06)
- Skip a Lighter market the venue holds no candles for instead of scanning its whole history for them. The listing-date probe already asks for every 1d candle from the floor to now; when that comes back empty — as it does for 19 listed-but-never-traded stock and FX perps (AAOI, ARM, AVGO, AXTI, BE, BYD, GME, KIOXIA, KORU, NOK, PONS, POPMART, QCOM, QNT, SHEIN, SOXS, TTWO, USDHKD, WDC; `/api/v1/candles` answers 200 with zero rows at every resolution) — the downloader fell back to walking the 1m history from the 2024-01-01 floor: 2825 windows at the 750 ms throttle, 35 minutes per symbol, eleven hours per 1m run, and nothing written at the end of it. An incremental run whose real work takes ten minutes ran for the better part of a day. The symbol is now skipped for the run with an info line; nothing is created, so nothing can be truncated, and the next run probes again for two requests
