...
- Probe below the stored history when recovering an unresolved MEXC Spot prefix. The probe was anchored at the marker's requested start, and because the venue answers a fixed 500-interval window from the query's `startTime`, it only ever inspected the oldest window of the range: empty for every symbol listed after that point, whatever sat higher up. Recovery therefore never converged. It now looks at the window immediately below the stored CSV, where an interrupted or provisionally bounded earlier download left off
- Apply the kernel-level connection bounds to Binance and Bybit as well, and stop the Bybit session documenting `SO_RCVTIMEO` as a working limit on POSIX, where a timed-out recv reports `EAGAIN` and Asio's synchronous path polls on without a deadline. The connect itself remains bounded only by the kernel's SYN retries; bounding it explicitly needs the request path rewritten to async operations
- Cover the run lock's identity with a regression test: the same exchange and output tree must resolve to one lock under any combination of `XDG_RUNTIME_DIR` and `TMPDIR`, two spellings of one tree must share it, two trees or exchanges must not, and contention must stay distinguishable from an unusable lock path
